...
/**
 * Annotation Processor for {@link interpolation.api.InterpolationMethod}.
 *
 * This demo processor does some validations and creates a class.
 */

@SpiService(Processor.class)
@DeclareCompilerMessageCodePrefix("InterpolationMethod")
public class InterpolationMethodProcessor extends AbstractAnnotationProcessor {

    private final static Set<String> SUPPORTED_ANNOTATIONS = createSupportedAnnotationSet(InterpolationMethod.class);
    ...
    public boolean processAnnotations(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        if (!roundEnv.processingOver()) {
            ...
            for (Element element : roundEnv.getElementsAnnotatedWith(InterpolationMethod.class)) {

                TypeElementWrapper wrappedTypeElement = TypeElementWrapper.wrap((TypeElement) element);
                InterpolationMethodWrapper annotation = InterpolationMethodWrapper.wrap(wrappedTypeElement.unwrap());
    ...
    void processAnnotation(TypeElementWrapper wrappedTypeElement, InterpolationMethodWrapper annotation) {
    ...
    boolean validateUsage(TypeElementWrapper wrappedTypeElement, InterpolationMethodWrapper annotation) {
        ...
        if(annotation.value().isEmpty()) {
            wrappedTypeElement.compilerMessage().asError().write(InterpolationMethodProcessorCompilerMessages.ERROR_VALUE_MUST_NOT_BE_EMPTY);
            result = false;
        }
        return result;
     ...
     * @param annotation The InterpolationMethod annotation
     */
    @DeclareCompilerMessage(code = "ERROR_001", enumValueName = "ERROR_COULD_NOT_CREATE_CLASS", message = "Could not create class ${0} : ${1}")
    private void createClass(TypeElementWrapper wrappedTypeElement, InterpolationMethodWrapper annotation) {
        ...
        try {
            ...
            javaWriter.writeTemplate("/InterpolationMethod.tpl", model);
            javaWriter.close();
        } catch (IOException e) {
            wrappedTypeElement.compilerMessage().asError().write(InterpolationMethodProcessorCompilerMessages.ERROR_COULD_NOT_CREATE_CLASS, filePath, e.getMessage());
        }
    }
